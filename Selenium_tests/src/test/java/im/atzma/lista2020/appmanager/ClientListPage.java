...
public class ClientListPage extends HelperBase {
    ...
    List<WebElement> number_of_client;
...
//        highlight(title_on_clienPage);
        if (isElementPresent(title_on_clienPage)) {
            System.out.println("true");
            return true;
        } else return false;
    }
    public boolean defaultNumberOfClient() throws InterruptedException {
        if(number_of_client.size() > 0) {
            return true;
        }
        return false;
    }
